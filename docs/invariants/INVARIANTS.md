...
nim-llm, Safe Synthesizer, and nim-embedding **coexist** on the H200 141GB via KAI fractional GPU allocation. Only nim-reasoning (120B) requires full-GPU preemption.

```yaml
# nim-llm (always-on, coexists with safe-synth)
priorityClassName: inference      # Priority 125
replicaCount: 1
kai.scheduler.nvidia.com/gpu-memory: "25Gi"

# safe-synth (always-on, coexists with nim-llm)
replicaCount: 1
kai.scheduler.nvidia.com/gpu-memory: "40Gi"
kai.scheduler/queue: inference-queue

# nim-embedding (always-on, coexists with all)
priorityClassName: build-preemptible  # Priority 75
replicaCount: 1
kai.scheduler.nvidia.com/gpu-memory: "2Gi"

# nim-reasoning (on-demand, preempts all — full GPU)
priorityClassName: inference      # Priority 125
replicaCount: 0                   # Scale up on demand
kai.scheduler.nvidia.com/gpu-memory: "141Gi"
```

**GPU Memory Budget:**
| Workload | KAI Annotation | Concurrent? |
|----------|---------------|-------------|
| nim-llm (8B) | 25Gi | Always-on |
| Safe Synth (1.1B) | 40Gi | Always-on |
| nim-embedding | 2Gi | Always-on |
| **Total** | **67Gi** | **<< 141Gi** |
| nim-reasoning (120B) | 141Gi | On-demand only (preempts all) |

**Rationale**: TinyLlama 1.1B (~10-15GB actual VRAM) and Llama 8B (~20GB actual) easily coexist on the H200 141GB. KAI fractional allocation manages scheduling. Only the 120B reasoning model needs full-GPU preemption.
...
### INV-N007: Safe Synthesizer Always-On (No Manual Scaling)

Safe Synthesizer runs always-on (`replicaCount=1`) alongside nim-llm. **NEVER manually scale** the Safe Synthesizer deployment — it coexists with nim-llm via KAI fractional GPU allocation.

```yaml
# Safe Synthesizer deployment (always-on)
...
# FORBIDDEN - manual scaling violates GitOps (INV-G003)
kubectl scale deployment nemo-safe-synthesizer --replicas=1  # NEVER
kubectl scale deployment nemo-safe-synthesizer --replicas=0  # NEVER
self._scale_deployment(replicas=1)  # NEVER from Dagster
```

**Rationale**: Manual scaling breaks GitOps (ArgoCD reverts), creates race conditions, and is unnecessary — Safe Synthesizer (TinyLlama 1.1B, ~10-15GB VRAM) easily coexists with nim-llm (8B, ~20GB VRAM) on the H200 141GB GPU.
