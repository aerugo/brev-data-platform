...
### INV-N004: GPU Co-Inference via KAI Fractional Allocation

nim-llm, Safe Synthesizer, nim-embedding, and nim-reasoning share the H200 141GB via KAI fractional GPU allocation. Dagster manages scaling to ensure combinations that exceed 141Gi never run simultaneously.

```yaml
# nim-llm (always-on, coexists with all services)
priorityClassName: inference      # Priority 125
replicaCount: 1
kai.scheduler.nvidia.com/gpu-memory: "25Gi"

# safe-synth (always-on, Dagster scales down during reasoning phases)
replicaCount: 1
kai.scheduler.nvidia.com/gpu-memory: "40Gi"
kai.scheduler/queue: inference-queue

# nim-embedding (always-on, coexists with all services)
priorityClassName: build-preemptible  # Priority 75
replicaCount: 1
kai.scheduler.nvidia.com/gpu-memory: "2Gi"

# nim-reasoning (on-demand, Dagster-managed scaling)
priorityClassName: inference      # Priority 125
replicaCount: 0                   # Scale up on demand via Dagster
kai.scheduler.nvidia.com/gpu-memory: "80Gi"
```

**GPU Memory Budget:**
| Combination | Total | Fits 141Gi? |
|---|---|---|
| nim-reasoning + nim-llm + nim-embedding | 80 + 25 + 2 = 107Gi | Yes |
| nim-llm + safe-synth + nim-embedding | 25 + 40 + 2 = 67Gi | Yes |
| nim-reasoning + safe-synth + nim-embedding | 80 + 40 + 2 = 122Gi | Yes |
| nim-reasoning + nim-llm + safe-synth + nim-embedding | 80 + 25 + 40 + 2 = 147Gi | **No** |

Dagster's `_ensure_nim_reasoning_ready` scales safe-synth to 0 before scaling nim-reasoning up, and restores safe-synth to 1 when nim-reasoning scales back to 0.

**Rationale**: GPT-OSS-120B (MoE, MXFP4) needs only ~80Gi, not the full 141Gi GPU. This allows nim-reasoning to coexist with nim-llm + nim-embedding. Dagster orchestrates the safe-synth/nim-reasoning swap to avoid exceeding GPU capacity.
...
### INV-N007: Safe Synthesizer Always-On (Dagster-Managed Exception for Reasoning)

Safe Synthesizer runs always-on (`replicaCount=1`) alongside nim-llm, **except** when nim-reasoning needs the GPU. Dagster's `_ensure_nim_reasoning_ready` scales safe-synth to 0 before scaling nim-reasoning up, and restores it when nim-reasoning scales back to 0.

```yaml
# Safe Synthesizer deployment (always-on by default)
replicaCount: 1                              # Always running
kai.scheduler/queue: inference-queue         # Same queue as nim-llm
kai.scheduler.nvidia.com/gpu-memory: "40Gi"  # Fractional GPU (coexists with 25Gi nim-llm)
```

```python
# CORRECT - call the always-on API directly
result = safe_synth.synthesize(input_data, run_id, config)

# CORRECT - Dagster K8sScalerResource manages safe-synth during reasoning phases
k8s_scaler.scale("nvidia-safe-synth-safe-synthesizer", "nvidia-ai", replicas=0)  # Before nim-reasoning up
k8s_scaler.scale("nvidia-safe-synth-safe-synthesizer", "nvidia-ai", replicas=1)  # After nim-reasoning down

# FORBIDDEN - manual kubectl scaling
kubectl scale deployment nvidia-safe-synth-safe-synthesizer --replicas=0  # NEVER via kubectl
kubectl scale deployment nvidia-safe-synth-safe-synthesizer --replicas=1  # NEVER via kubectl
```

**Rationale**: nim-reasoning (80Gi) + nim-llm (25Gi) + safe-synth (40Gi) + nim-embedding (2Gi) = 147Gi exceeds the H200's 141Gi. Dagster orchestrates the swap gracefully — no manual kubectl scaling, no mid-request preemption.
